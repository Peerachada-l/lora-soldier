...
const StatusPage = ({ soldiers, onAddSoldier }) => {
    ...
    const accentColor = (() => {
        ...
        if (rankFilter !== 'All Ranks') return 'text-purple-400 border-purple-500';
                            ...
                            <div className="absolute right-0 mt-2 bg-slate-800 border border-slate-700 rounded-lg shadow-lg w-40 z-20">
                                {units.map(option => (
                                    <div
                                        ...
                                        className={`px-4 py-2 cursor-pointer hover:bg-slate-700 transition ${unitFilter === option ? 'bg-slate-700 text-blue-400 font-semibold' : 'text-slate-300'
                                            }`}
                                    >
                                        {option}
                                    </div>
                            ...
                            <div className="absolute right-0 mt-2 bg-slate-800 border border-slate-700 rounded-lg shadow-lg w-40 z-20">
                                {ranks.map(option => (
                                    <div
                                        ...
                                        className={`px-4 py-2 cursor-pointer hover:bg-slate-700 transition ${rankFilter === option ? 'bg-slate-700 text-blue-400 font-semibold' : 'text-slate-300'
                                            }`}
                                    >
                                        {option}
                                    </div>
